...
void *clientHandler(void *chv)
{
	...
	if(strcmp(msgBuffer, "/user\n") == 0){
        ...
        {
	    //write(clientSocketFd, "to who?\n", MAX_BUFFER - 1);
	  
	    //fprintf(stderr,"to %s\n",msgBuffer);
	    //do{
	    read(clientSocketFd, msgBuffer, MAX_BUFFER - 1);//2
	    //}while(strcmp(msgBuffer, "a") < 0);	    
